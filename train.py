...
def main():
    ...
    for episode in range(300):
        ...
        while not done:
            ...
            new_grid, reward, done, object_spawn_interval = step(agent, action, grid, object_spawn_interval, episode_time_limit)
            
            object_spawn_interval = object_spawn_interval

            grid = draw(SCREEN, new_grid, ROWS, COLUMNS, WIN_WIDTH, WIN_HEIGHT)
            agent.draw(SCREEN)
            pygame.display.update()
            fpsclock.tick(fps)

            agent.score += reward
            replay_memory.append([grid, agent, action, agent.score, new_grid, done])
